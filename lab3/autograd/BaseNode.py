...
class Linear(Node):
    ...
    def backcal(self, grad):
        ...
        '''
        weight = self.params[0]
        # print(self.cache[-1].shape,grad.shape,weight.shape)
        self.grad.append(self.cache[-1].T @ grad)
        self.grad.append(np.sum(grad,axis=0))
        # print(grad.shape, np.sum(grad,axis=0).shape)
...
class Softmax(Node):
    ...
    def cal(self, X):
        X = X - np.max(X, axis=self.dim, keepdims=True) # 防止指数爆炸
        expX = np.exp(X)
...
class LogSoftmax(Node):
    ...
    def backcal(self, grad):
        Logsoftmax = self.cache[-1]
        return grad - np.multiply(grad.sum(axis=self.dim, keepdims=True),np.exp(Logsoftmax))
